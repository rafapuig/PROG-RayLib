...
final float gravity = 2_000f; // pixels / segundo ^ 2
...
void main() {
    ...
    float ballPositionX = screenWidth / 2f;
    float ballPositionY = screenHeight * .2f;
    ...
    while (!WindowShouldClose()) {
        ...
        if (!paused) {
            ...
            ballPositionX = ballPositionX + velocityX * deltaTime;
            ballPositionY = ballPositionY + velocityY * deltaTime;

            // Incrementamos la velocidad en Y si se usa la gravedad
            // ∆(V.y) = gravity * ∆(t) --> V.y = V.y + gravity * deltaTime
            if (useGravity) velocityY += gravity * deltaTime;

            if (ballPositionX <= radius || ballPositionX >= GetScreenWidth() - radius) {
                velocityX *= -1f;
            }

            if (ballPositionY <= radius || ballPositionY >= GetScreenHeight() - radius) {
                velocityY *= -0.95f; // Por fricción pierde un 5%
            }

            ballPositionX = Math.clamp(ballPositionX, radius, GetScreenWidth() - radius);
            ballPositionY = Math.clamp(ballPositionY, radius, GetScreenHeight() - radius);
        }


        BeginDrawing();
        ClearBackground(DARKGRAY);
        DrawCircle((int) ballPositionX, (int) ballPositionY, radius, ballColor);
