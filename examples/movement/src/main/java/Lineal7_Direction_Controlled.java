...
void main() {
    ...
    while (!WindowShouldClose()) {
        ...
        boolean isTurning = isTurningLeft || isTurningRight;

        if (isTurning) {
            turnSpeed = speed / maxSpeed * maxTurnSpeed;

            /*if (isTurningLeft) angle -= turnSpeed;
            if (isTurningRight) angle += turnSpeed;*/
            angle += turnSpeed * (isTurningLeft ? -1 : 1);
        }
